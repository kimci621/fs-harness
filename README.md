...
Три вкладки: MR, задачи Jira, запуски. `tab` и `1-3` переключают, `j`/`k` и стрелки ходят по списку,
детали выбранной строки видны справа. По выбранной строке запускаются действия: `a` — conflict, `t` — threads, `r` — review,
`n` — analyze, `s` — сменить статус задачи (выбор перехода списком). Запуск становится карточкой с фазой, ценой и последней строкой агента, а полный вывод
