...
- `mattermost` — `webhook` (incoming webhook канала). Пустой — уведомления просто не шлются.
...
| `jira [mine\|<KEY>\|move <KEY> <статус>]` | Задачи Jira: список с фильтрами (`--assignee`, `--sprint`, `--component`, `--status`, `--jql`), одна задача с комментариями, смена статуса |
...
| `watch` | Один опрос: что изменилось в MR с прошлого раза, триаж судьёй, уведомление в Mattermost. Действия не запускает |
| `tui` | Полноэкранный режим: вкладки MR / задачи / история, действия и пайплайны с клавиши, живой лог агента и судьи |
...
Три вкладки: MR, задачи Jira, история запусков (что запускали раньше: вердикт, цена, каталог рана).
`tab` и `1-3` переключают, `j`/`k` и стрелки ходят по списку, детали выбранной строки видны справа.
В списке MR сразу видно `⚠` — есть конфликт, `💬N` — столько тредов не закрыто, и иконку пайплайна.

По выбранной строке запускаются действия: `a` — решить конфликт, `t` — обработать тикеты (треды),
`r` — локальное ревью, `n` — разбор задачи, `s` — сменить статус задачи (выбор перехода списком).
`p` открывает панель пайплайна: все джобы по стадиям, как в GitLab, `enter` запускает выбранную.
Любой `deploy_dev*` сам сперва прогонит `build_image`, если тот ещё не собран, и дождётся его.
...
```bash
fsh watch          # один опрос: снимок → дифф с прошлым → триаж → уведомление
```
...
Первый опрос ничего не показывает: сравнивать не с чем. Запускает опрос человек — расписания,
демона и автозапуска действий здесь нет и не будет (PLAN, «чего в v1 не делаем», п. 6).

## Уведомления в Mattermost

Задан `mattermost.webhook` в конфиге (можно на проект) — по завершении каждого действия в канал
уходит одна строка: что делали, чем кончилось, решение судьи, цена и ссылка на MR или задачу.
Ничего, кроме webhook, не нужно: бота, токена и websocket здесь нет. Не ответил Mattermost — ран это
не ломает, в лог падает `⚠ Уведомление не ушло`.
