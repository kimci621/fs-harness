...
├──────────────────────┬───────────────────────────────────────┤
│ !2547 ⚠ 💬3 ✅ fix/b │ MR !2547 · fix/banner → dev           │
│ !2551 💬1 🔵 feat/pr │ pipeline ✅ · треды 3/7 · конфликт ⚠  │
│ !2559 ⏸ fix/checkout │ ───────────────────────────────────── │
│                      │ p — пайплайн и запуск джоб            │
├──────────────────────┴───────────────────────────────────────┤
...
Клавиши: `1/2/3` и `Tab` - вкладки; `↑↓`/`jk` - курсор, `PgUp/PgDn` - на десять строк;
`a/t/r/n` - запустить conflict/threads/review/analyze для выделенного; `s` - сменить статус задачи;
`p` - панель пайплайна MR; `x` - прервать ран (тот самый `AbortSignal`); `o` - открыть `web_url`;
...
слово «раны» читалось как «ранеры». Строка MR несёт маркеры `⚠` (конфликт) и `💬N` (открытые треды),
чтобы не открывать детали ради одного факта. Не сделаны и пока не нужны: `Enter` - детали (правая
панель и так показывает выделенное), `Space` - свернуть лог, `v` - вердикт целиком, `/` - фильтр
(фильтры живут флагами CLI).
...
4. Комментарии и worklog в Jira - слой read-only. *Изменено по решению владельца 2026-09-11:*
   смена статуса (`fsh jira move`, клавиша `s` в TUI) разрешена и сделана - это единственная запись
   в Jira. Переходы с обязательными полями workflow-валидатора (в FD это «В ревью») командой не
   закрываются: Jira возвращает список недостающих полей, их по-прежнему заполняет скилл
   `jira-transitions`.
