...
Статус: фазы 0-15, 17, 18 и 19 сделаны. Фаза 16 отложена до решения владельца.
...
| 18. Бэкенд в контуре задачи | сделано | [PLAN-FAZA-18.md](PLAN-FAZA-18.md) |
...
выполнения: 18 сделана (16 отложена).
...
2. **Промпты действий.** Переменные `backend_dir` и `backend_repo` в шапке промптов, где агент
   пишет код или разбирает задачу (`analyze`, `ci-fix`, `implement-plan`, `implement-execute`). Текст короткий:
...
4. **Судья видит поход.** В `facts` приёмки добавляется `backend_refs` - файлы бэкенда, на которые
   агент сослался (`readBackendRefs` читает `<runDir>/backend_refs.json`). Пусто - не приговор (бывают задачи без API), но у судьи это перед глазами.
...
*Сделано:*
- Поле `projects.<имя>.backend`, `resolveProject()` и `cfg.backend` в `src/config.js`, проверка каталога в `src/commands/doctor.js`.
- Блок бэкенда и переменные `backend_dir`, `backend_repo` в `analyze.md`, `ci-fix.md`, `implement-plan.md`, `implement-execute.md` и в `context(x)` действий.
- Сценарий `src/prompts/flows/backend-check.md` и ссылка в `take-task.md`.
- `readBackendRefs` в `actions/analyze.js` и сбор `facts.backend_refs` в `analyze` и `implement`.
- Проект `fitstars-api4` в рабочем конфиге, проверен `-P fitstars-api4 mrs` и отказ без Jira.
