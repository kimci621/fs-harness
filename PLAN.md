...
**Сценарии живут в харнессе.** Новый `kind: 'flow'` в реестре: запись без `run`, с телом из
`src/prompts/flows/<имя>.md`. `fsh flow list` перечисляет, `fsh flow show <имя>` печатает протокол.
Три штуки: `take-task` (взять задачу в работу), `push-task` (запушить и открыть MR), `fill-jira`
(заполнить обязательные поля). Протокол - это markdown с шагами и точными командами `fsh`, а не
код: шаги меняются чаще, чем движок. Тест греплит из каждого протокола строки `fsh <команда>` и
падает, если такой команды в реестре нет.
...
проекта. `fsh init --check` говорит, разошёлся ли записанный скилл с нынешним реестром. SessionStart
hook не берём: он жрёт контекст каждой сессии, даже когда задача к Jira не относится.
...
`fsh growthbook list|get|create|toggle` - больше в v1 не нужно. Это удалённый HTTP-сервис, а не
программа: ограничение «ноль новых внешних программ» про то, что надо ставить рядом с проектом, и
GrowthBook его не нарушает - как не нарушают Jira и GitLab.
...
2. *Сценарии.* `kind: 'flow'`, `flow list|show`, протоколы `take-task` / `push-task` / `fill-jira`,
   раздел «Сценарии» в `agent-guide`, тест на то, что все упомянутые команды существуют.
3. *Инициализация.* `fsh init`, `--check`, проверка в `doctor`, установка в fitstars-frontend плюс
   `Bash(fsh:*)` в его `.claude/settings.json`.
4. *Приёмка.* `fsh task judge <KEY>`, рубрика `src/prompts/judge/task-acceptance.md`, роль в конфиге.
