...
по `Tab`, как в узком терминале. Мышь по-прежнему не делаем (см. «чего в v1 не делаем», п. 11). Уже 100 колонок две панели рядом не читаются, поэтому на узком терминале
...
4. Комментарии и worklog в Jira - слой read-only. *Изменено по решению владельца 2026-09-11 и
   ...
   Assignee, «Ответственный разработчик», Priority, «Компонент», Fix versions, Labels. Worklog и
   правка описания по-прежнему не наши. Переходы с обязательными полями
