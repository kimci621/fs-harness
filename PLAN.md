...
карточке переносятся по словам, а не обрезаются: поля-строки по-прежнему обрезает ink. Уже 100 колонок две панели рядом не читаются, поэтому на узком терминале
