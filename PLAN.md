...
В реестре одна data-запись `jira` с подкомандами (`jira mine`, `jira FD-7647`). Ключ детектится
`/^[A-Z]+-\d+$/` - это `target: 'issue'`.

**Смена статуса не изобретается:** в проекте уже есть скилл `jira-transitions`, знающий обязательные
поля workflow-валидатора (`customfield_10275`, `customfield_10242`, формат ADF). Действие зовёт его
через `claude -p "/jira-transitions ..."`, а не дублирует знание о полях.
...
│ [1] MR   [2] Задачи   [3] Раны                    ? помощь  │
├──────────────────────┬───────────────────────────────────────┤
│ !2547 fix/banner  ⚠  │ MR !2547 · fix/banner → dev           │
│ !2551 feat/promo  ✅ │ pipeline ✅ · треды 3/7 · конфликт ⚠  │
│ !2559 fix/checkout🔵 │ ───────────────────────────────────── │
│                      │ a конфликт   t треды   r ревью        │
├──────────────────────┴───────────────────────────────────────┤
...
Клавиши: `1/2/3` и `Tab` - вкладки; `↑↓`/`jk` - курсор; `Enter` - детали; `a/t/r/n` - запустить
conflict/threads/review/analyze для выделенного; `Space` - свернуть лог; `PgUp/PgDn` - скролл;
`v` - вердикт целиком; `x` - прервать ран (тот самый `AbortSignal`); `o` - открыть `web_url`;
`p` - открыть промпт действия в `$EDITOR` (с выходом из alt-screen); `/` - фильтр; `?` - помощь;
`q` - выход (при активных ранах подтверждение).
...
косметики значит ломать рабочий судейский слой. `judge.enabled` и `maxRevise` не введены: retry на
`revise` живёт в фазе 6, там же и появится флаг. Ленивый кэш `g` на проект отложен до TUI - сейчас
его некому звать.
...
4. Любые записи в Jira (переходы, комментарии, worklog) - слой read-only.
